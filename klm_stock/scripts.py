...
def list_scripts():
    ...
    for rec in res:
        print(f"{i}. [{rec[0]:3}] {rec[1]:15} - {rec[2]:50} :"
              f" ({rec[3]})")
        i += 1
...
def add_script():
    ...
    if sectors.get_sector_full_name(code=sec_code) is None:
        # we use sectors.get_sector_id to look up
        # the sector code's full name
        # if None is returned, it means
        # the given sector code is invalid.
...
def edit_script():
    ...
    :return: None
    """
    scr_code = input_script_code(check_exiting=True,
                                 prompt="Enter Script Code to Edit")
    if scr_code is None:
        return
    scr_new_code = input_script_code(check_exiting=False,
                                     prompt="Enter NEW Script Code")
    ...
    cmd = "UPDATE scripts SET script_code=%s, script_name=%s," \
...
def del_script():
    ...
    # First, check if records exists in
    # transaction table for the script
    cmd = "SELECT COUNT(t.id) FROM transactions t" \
    ...
    if rec_cnt[0] > 0:
        print(f"Can not delete as {rec_cnt[0]} transactions exist.")
        return
...
def input_script_code(*, check_exiting=False, prompt=None):
    ...
    If check_existing is true, the script code is
    checked if it already exists in table.
    If exists, the code is returned. If not exists,
    None is returned.
    """
    ...
    if len(scr_code) < MIN_SCRIPT_CODE_LEN:
        print(f"Invalid script code, must be "
              f"{MIN_SCRIPT_CODE_LEN} characters or more.")
        return
...
def input_script_name(prompt=None):
    ...
    if len(scr_name) < MIN_SCRIPT_NAME_LEN:
        print(f"Script name must be min {MIN_SCRIPT_NAME_LEN} characters.")
        return None
    return scr_name
